...
class ParticleProb : public TObject {
 ...
 public:
  ...
  ParticleProb(int type, int charge, int nhits, int truncmode, int probmode, TH2F* hprobabilities) {
    particle_type_ = type;
    charge_ = charge;
    nhits_ = nhits, truncmode_ = truncmode, probmode_ = probmode, hprobabilities_ = hprobabilities;
  };

  void Update(int type, int charge, int nhits, int truncmode, int probmode, TH2F* hprobabilities) {
    particle_type_ = type;
    charge_ = charge;
    nhits_ = nhits, truncmode_ = truncmode, probmode_ = probmode, hprobabilities_ = hprobabilities;
  };
  ...
  int GetId() {
    int type_pm = charge_ > 0 ? particle_type_ : particle_type_ + NumberOfPidsTrd - 1;
